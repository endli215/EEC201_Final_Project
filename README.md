...
According to Figure 1, the first step of the process is to generate the spectrumgram of the speech signal. To do that, we imported the audio file from the GitHub and plotted it in the time domain, which can help illustrate what is happening with the signal. In figure two, the left-hand side are the original audio real-time signal and spectrumgram. From the real-time signal, it is clear that most of the time, amplitudes are really low and almost stationary. The stationary input can make it more difficult to spot the difference in the spectrumgram. For example, in S12.wav, after trimming off the stationary part of the audio, the spectrumgram has become more unique and easier to be distinguished. 
<div align="center">
