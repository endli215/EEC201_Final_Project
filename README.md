...
According to Figure 1, the first step of the process is to generate the spectrumgram of the speech signal. To do that, we imported the audio file from the GitHub and plotted it in the time domain, which can help illustrate what is happening with the signal. In figure two, the left-hand side are the original audio real-time signal and spectrumgram. From the real-time signal, it is clear that most of the time, amplitudes are really low and almost stationary. The stationary input can make it more difficult to spot the difference in the spectrumgram. For example, in S12.wav and S11.Wave, etc, after trimming off the stationary part of the audio, the spectrumgram has become more unique and easier to be distinguished. So, we have decided before sending the signal into the frame blocking and windowing for the spectrumgram, we trimmed off the audio signal that was lower than 5% of the peak of the audio that lasted more than 0.2 seconds. Which the sounded letter is only thing used for speech reconization. 
To make the spectrumgram signal and graph, we used the Python function ```scipy.signal.stft``` and the input parameter you can choose the sample of each frame N, number of overlap sample N-M type of windowing and last but not least the fft point. In our test 2, we used N=256, N =100(156 overlaps), Kaiser window, and 512 fft point. This function will output the STFT of the time signal, and we can use that data to plot out the spectrumgram, as shown in Figure 2. In the next few tests, we have also calculated the STFT step by step by doing frame blocking, windowing, and FFT without using ```scipy.signal.stft``` and we were able to replicate the similar result. 
<div align="center">
